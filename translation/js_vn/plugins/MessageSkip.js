...
// 1.18.2 2023/06/03 Công tắc Bỏ qua、Công tắc Tự độngが設定されているとキーによるスキップオートが効かなくなる問題を修正
...
// 1.17.0 2021/09/12 ピクチャによるクリックはNhấn giữ để Bỏ quaの対象外とするよう仕様をパラメータで選択可能にできるよう修正
// 1.16.0 2021/09/08 スキップモードのときウェイトもスキップできる機能を追加
// 1.15.1 2021/08/05 カスタムメニュー作成プラグインと併用したときにエラーが発生する現象を修正
// 1.15.0 2021/06/15 ピクチャによるクリックはNhấn giữ để Bỏ quaの対象外とするよう仕様変更
// 1.14.1 2020/09/02 MZ向けにコードとヘルプ修正
// 1.14.0 2020/08/02 クリックすることで任意のスイッチをONにできるピクチャをメッセージウィンドウに表示する機能を追加
// 1.13.0 2020/03/26 オート、Hình ảnh Bỏ quaの表示方法をメッセージウィンドウからの相対座標と絶対座標とを選択できる機能を追加
// 1.12.1 2020/03/25 アイコン表示位置をメッセージウィンドウの位置やサイズの変更に追従するよう修正
// 1.12.0 2019/05/26 オート、Biểu tượng Bỏ quaの位置を自由に指定できる機能を追加
...
// 1.8.0 2018/02/16 Số frame chờ Tự động数の計算式にウィンドウに表示した文字数を組み込める機能を追加
// 1.7.0 2017/12/12 SkipAlreadyReadMessage.jsとの連携したときに当プラグインのスキップ機能が既読スキップになるよう修正
//                  Hình ảnh Bỏ quaの条件スイッチが0(指定なし)のときに同ピクチャが表示されない問題を修正
...
//                  Số frame chờ Tự độngを制御文字を使って動的に変更できる機能を追加
 ...
 * @desc ピクチャはNhấn giữ để Bỏ quaの対象外とします。
 * @default true
 * @type boolean
 *
 * @param AutoWaitFrame
 ...
 *
 * @param SkipPicture
 * @desc ウィンドウ内に表示するHình ảnh Bỏ quaのファイル名です。クリックするとスキップモードになります。
 * @default
 * @require 1
 * @dir img/pictures/
 * @type file
 *
 * @param SkipPictureX
 * @desc ウィンドウ内に表示するHình ảnh Bỏ quaのX座標です。
 * @default 500
 * @type number
 *
 * @param SkipPictureY
 * @desc ウィンドウ内に表示するHình ảnh Bỏ quaのY座標です。
 * @default 0
 * @type number
 *
 * @param AutoPicture
 * @desc ウィンドウ内に表示するHình ảnh Tự độngのファイル名です。クリックするとオートモードになります。
 * @default
 * @require 1
 * @dir img/pictures/
 * @type file
 *
 * @param AutoPictureX
 * @desc ウィンドウ内に表示するHình ảnh Tự độngのX座標です。
 * @default 750
 * @type number
 *
 * @param AutoPictureY
 * @desc ウィンドウ内に表示するHình ảnh Tự độngのY座標です。
 * @default 0
 * @type number
 *
 * @param SwitchPicture
 * @desc ウィンドウ内に表示するHình ảnh Công tắcのファイル名です。クリックすると指定したスイッチがONになります。
 * @default
 * @require 1
 * @dir img/pictures/
 * @type file
 ...
 * @desc Hình ảnh Công tắcをクリックしたときにONにするスイッチ番号です。
 * @default 0
 * @type switch
 *
 * @param SwitchPictureX
 * @desc ウィンドウ内に表示するHình ảnh Công tắcのX座標です。
 * @default 750
 * @type number
 *
 * @param SwitchPictureY
 * @desc ウィンドウ内に表示するHình ảnh Công tắcのY座標です。
 * @default 0
 * @type number
 *
 ...
 * @desc オート、Hình ảnh Bỏ quaの配置方法です。相対座標を選択するとウィンドウ表示位置からの相対座標となります。
 * @default relative
 * @type select
 * @option absolute
 * @option relative
 ...
 *
 * ・パラメータ「Số frame chờ Tự động」を設定するとオートモード時の待機フレームを変更できます。
 ...
 * @option A
 *
 * @param Công tắc Bỏ qua
 * @desc 指定した番号のスイッチがONになっている場合は常にスキップします。OFFになると止まります。
 * @default 0
 * @type switch
 *
 * @param Công tắc Tự động
 * @desc 指定した番号のスイッチがONになっている場合は常にオートします。スキップが優先されます。OFFになると止まります。
 * @default 0
 * @type switch
 *
 * @param Biểu tượng Bỏ qua
 * @desc メッセージスキップ中にウィンドウ右下に表示されるアイコン
 * @default 140
 * @type number
 *
 * @param Biểu tượng Tự động
 * @desc メッセージオート中にウィンドウ右下に表示されるアイコン
 * @default 75
 * @type number
 *
 * @param Tọa độ X biểu tượng
 * @desc オート、スキップのアイコン位置を自由に変更したい場合に指定するX座標です。
 * @default 0
 * @type number
 * @min -2000
 ...
 * @max 2000
 *
 * @param Nhấn giữ để Bỏ qua
 * @desc スキップの判定が指定のキーを押している間のみになります。
 * @default false
 * @type boolean
 *
 * @param Không áp dụng nhấn giữ cho hình ảnh
 * @desc ピクチャはNhấn giữ để Bỏ quaの対象外とします。
 * @default true
 * @type boolean
 *
 * @param Số frame chờ Tự động
 * @desc オートモードが有効の場合にメッセージを表示しておくフレーム数。制御文字\v[n]および計算式が指定できます。
 * @default 100 + textSize * 10
 *
 * @param ID công tắc hủy khi hoàn thành
 * @desc 指定した番号のスイッチがONのとき、イベント終了時にスキップ、オート状態を解除します。0の場合は常に解除します。
 * @default 0
 * @type switch
 *
 * @param Hình ảnh Bỏ qua
 * @desc ウィンドウ内に表示するHình ảnh Bỏ quaのファイル名です。クリックするとスキップモードになります。
 * @default
 * @require 1
 * @dir img/pictures/
 * @type file
 *
 * @param Hình ảnh Bỏ quaX
 * @desc ウィンドウ内に表示するHình ảnh Bỏ quaのX座標です。
 * @default 500
 * @type number
 *
 * @param Hình ảnh Bỏ quaY
 * @desc ウィンドウ内に表示するHình ảnh Bỏ quaのY座標です。
 * @default 0
 * @type number
 *
 * @param Hình ảnh Tự động
 * @desc ウィンドウ内に表示するHình ảnh Tự độngのファイル名です。クリックするとオートモードになります。
 * @default
 * @require 1
 * @dir img/pictures/
 * @type file
 *
 * @param Hình ảnh Tự độngX
 * @desc ウィンドウ内に表示するHình ảnh Tự độngのX座標です。
 * @default 750
 * @type number
 *
 * @param Hình ảnh Tự độngY
 * @desc ウィンドウ内に表示するHình ảnh Tự độngのY座標です。
 * @default 0
 * @type number
 *
 * @param Hình ảnh Công tắc
 * @desc ウィンドウ内に表示するHình ảnh Công tắcのファイル名です。クリックすると指定したスイッチがONになります。
 * @default
 * @require 1
 * @dir img/pictures/
 * @type file
 *
 * @param Hình ảnh Công tắcトリガー
 * @desc Hình ảnh Công tắcをクリックしたときにONにするスイッチ番号です。
 * @default 0
 * @type switch
 *
 * @param Hình ảnh Công tắcX
 * @desc ウィンドウ内に表示するHình ảnh Công tắcのX座標です。
 * @default 750
 * @type number
 *
 * @param Hình ảnh Công tắcY
 * @desc ウィンドウ内に表示するHình ảnh Công tắcのY座標です。
 * @default 0
 * @type number
 *
 * @param Gốc tọa độ nút
 * @desc スキップ、オートの各ピクチャボタン座標の原点です。メッセージウィンドウの四隅が基準座標になります。
 * @default 0
 * @type select
 * @option ウィンドウの左上
 ...
 * @value 3
 *
 * @param ID công tắc hiển thị nút
 * @desc 指定した番号のスイッチがONのときのみスキップ、オートの各ピクチャボタンを表示します。0の場合は無条件で表示します。
 * @default 0
 * @type switch
 *
 * @param Loại tọa độ hình ảnh
 * @desc オート、Hình ảnh Bỏ quaの配置方法です。相対座標を選択するとウィンドウ表示位置からの相対座標となります。
 * @default relative
 * @type select
 * @option 絶対座標
...
(function() {
    ...
    // パラメータの取得と整形
    //=============================================================================
    var skipKeyName = getParamString(['SkipKey', 'Phím Bỏ qua', 'Phím Bỏ qua (Skip)']).toLowerCase();
    var skipKeyCode = Input.keyCodeReverseMapper[skipKeyName];
    var autoKeyName = getParamString(['AutoKey', 'Phím Tự động', 'Phím Tự động (Auto)']).toLowerCase();
    ...
    var paramSkipPicture          = getParamString(['SkipPicture', 'Hình ảnh Bỏ qua']);
    var paramSkipPictureX         = getParamNumber(['SkipPictureX', 'Hình ảnh Bỏ quaX']);
    var paramSkipPictureY         = getParamNumber(['SkipPictureY', 'Hình ảnh Bỏ quaY']);
    var paramAutoPicture          = getParamString(['AutoPicture', 'Hình ảnh Tự động']);
    var paramAutoPictureX         = getParamNumber(['AutoPictureX', 'Hình ảnh Tự độngX']);
    var paramAutoPictureY         = getParamNumber(['AutoPictureY', 'Hình ảnh Tự độngY']);
    var paramSwitchPicture        = getParamString(['SwitchPicture', 'Hình ảnh Công tắc']);
    var paramSwitchPictureX       = getParamNumber(['SwitchPictureX', 'Hình ảnh Công tắcX']);
    var paramSwitchPictureY       = getParamNumber(['SwitchPictureY', 'Hình ảnh Công tắcY']);
    var paramSwitchPictureTrigger = getParamNumber(['SwitchPictureTrigger', 'Hình ảnh Công tắcトリガー'], 0);
    var paramPictureAnchor        = getParamNumber(['PictureAnchor', 'Gốc tọa độ nút']);
    var paramPictureSwitchId      = getParamNumber(['PictureSwitchId', 'ID công tắc hiển thị nút'], 0);
    var paramPressingSkip         = getParamBoolean(['PressingSkip', 'Nhấn giữ để Bỏ qua']);
    var paramPictureOutOfPressing = getParamBoolean(['PictureOutOfPressing', 'Không áp dụng nhấn giữ cho hình ảnh']);
    var paramSkipSwitchId         = getParamNumber(['SkipSwitchId', 'Công tắc Bỏ qua'], 0);
    var paramAutoSwitchIId        = getParamNumber(['AutoSwitchIId', 'Công tắc Tự động'], 0);
    var paramInvalidSwitchId      = getParamNumber(['InvalidSwitchId', 'Công tắc Vô hiệu hóa'], 0);
    var paramIconX                = getParamNumber(['IconX', 'Tọa độ X biểu tượng'], 0);
    var paramIconY                = getParamNumber(['IconY', 'Tọa độ Y biểu tượng'], 0);
    var paramPicturePosType       = getParamString(['PicturePosType', 'Loại tọa độ hình ảnh']);
    ...
    Game_Message.prototype.initialize = function() {
        ...
        this.clearSkipInfo();
        this._autoClearSkipSwitch = getParamNumber(['ResetOnEndSwitch', 'ID công tắc hủy khi hoàn thành']);
    };
    ...
    Game_Interpreter.prototype.isNeedClearSkip = function() {
        ...
        if($gameSwitches.value(1000) == true){return false};//フラグクリアキャンセル
    ...
    Window_Message.prototype.initializeMessageAutoCount = function() {
        ...
        var paramValue         = convertEscapeCharacters(getParamString(['AutoWaitFrame', 'Số frame chờ Tự động'])) || 1;
    ...
    Window_Message.prototype.updateAutoIcon = function() {
        if (this.messageSkip() && this.openness === 255) {
            this._icon.refresh(getParamNumber(['SkipIcon', 'Biểu tượng Bỏ qua']));
            this._icon.flashSpeed = 16;
            this._icon.flash      = true;
            this.updatePlacementIcon();
        } else if (this.messageAuto() && this.openness === 255) {
            this._icon.refresh(getParamNumber(['AutoIcon', 'Biểu tượng Tự động']));
    ...
    Window_Message.prototype.setSkipAutoFlagByTrigger = function() {        
        if (this.isTriggeredMessageSkip()) {        
            ...
            const isHeld = this.isPressedMessageSkip() || this.isCustomSkipIconPressed();
            $gameMessage.setSkipFlg(isHeld);
