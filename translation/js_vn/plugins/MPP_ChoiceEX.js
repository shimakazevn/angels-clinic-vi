 ...
 *  〇 MV / MZ
 *  
 *  〇 ChoicePos x y row  / Cài đặt vị trí lựa chọn
 *       x   : 選択肢ウィンドウのX座標
 *       y   : 選択肢ウィンドウのY座標
 *       row : 選択肢ウィンドウの行数。未設定の場合はすべて表示
 *   - 次に表示する選択肢の位置(x,y)と行数(row)指定します。
 * 
 *  〇 ChoiceVariableId varId  / Cài đặt vị trí con trỏ
 *       varId : 変数番号
 ...
 *     選択肢毎に+10されます。
 * 
 *  〇 ChoiceRect x y width height  / Cài đặt kích thước lựa chọn
 *       x      : X座標
 *       y      : Y座標
 *       width  : 幅
 *       height : 高さ
 ...
 *   
 *  〇 ChoiceInMessage  / Hiển thị trong khung tin nhắn
 ...
 *
 *  @command choiceRect
 *      @text Cài đặt kích thước lựa chọn
 ...
 *
 *  @command choiceInMessage
 *      @text Hiển thị trong khung tin nhắn
...
(() => {
    ...
    PluginManager[_registerCommandName](pluginName, 'choicePos', args => {
    ...
    PluginManager[_registerCommandName](pluginName, 'choiceVariableId', args => {
        const variableId = PluginManager.mppValue(args.varId);
        $gameMessage.setChoiceVariableId(variableId);
    });

    PluginManager[_registerCommandName](pluginName, 'choiceRect', args => {
    ...
    PluginManager[_registerCommandName](pluginName, 'choiceInMessage', () => {
    ...
    Game_Interpreter.prototype.evalChoice = function(condition) {
        try {
            ...
            console.log('Lỗi điều kiện' + condition);
            return true;
        }
    };
    ...
    Object.assign(_mzCommands, {
        'Cài đặt vị trí lựa chọn': _mzCommands.ChoicePos,
        'Cài đặt vị trí con trỏ': _mzCommands.ChoiceVariableId,
        'Cài đặt kích thước lựa chọn': _mzCommands.ChoiceRect,
        'Hiển thị trong khung tin nhắn': _mzCommands.ChoiceInMessage
    });
    ...
    Window_Message.prototype.clearInChoice = function() {
        if (this._textState && $gameMessage.isChoiceInMessage()) {
