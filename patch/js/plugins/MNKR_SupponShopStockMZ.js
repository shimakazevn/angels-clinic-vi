 ...
 * - ショップの起動
 * - ショップの削除
 * - 変数管理しないアイテムのSố lượng còn:を、指定して変数に代入
 *
 * 
 * このプラグインについて
 ...
 * @text Số lượng còn:の表記
 * @desc Số lượng còn:の表記を設定します
 * @default Số lượng còn:
 *
 * @param Label of sold out
 * @text Hết hàngの表記
 * @desc Hết hàngの表記を設定します
 * @default Hết hàng
 *
 * @param Display option
 * @text Số lượng còn:を表示
 * @type boolean
 * @on Số lượng còn:を表示
 * @off Số lượng còn:を非表示
 * @desc true にするとSố lượng còn:が、持っている数の横に表示されます。
 * @default false
 *
 * @param Display option space
 * @text オプション表示スペース
 * @desc 持っている数とSố lượng còn:の表示スペースです。
 * @default 12
 * 
 * 
 * @command makeShop
...
(function () {
    ...
    var StockLabel = String(parameters['Label of stock Number'] || 'Số lượng còn:');
    var SoldOutLabel = String(parameters['Label of sold out'] || 'Hết hàng');
